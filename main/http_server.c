 ...
 * @return ESP_OK
 */
static esp_err_t http_server_get_data_sensor_readings_json_handler(httpd_req_t *req)
{
	ESP_LOGI(TAG, "/dataSensor.json requested");

	char dataSensorJSON[100];

	sprintf(dataSensorJSON, "{\"temp_obj\":\"%.1f\",\"temp_ambient\":\"%.1f\"}", note_temp_object, note_tempt_ambient);

	httpd_resp_set_type(req, "application/json");
	httpd_resp_send(req, dataSensorJSON, strlen(dataSensorJSON));
...
static httpd_handle_t http_server_configure(void)
{
	...
	if (httpd_start(&http_server_handle, &config) == ESP_OK)
	{
		...
		httpd_uri_t data_sensor_json = {
				.uri = "/dataSensor.json",
				.method = HTTP_GET,
				.handler = http_server_get_data_sensor_readings_json_handler,
				.user_ctx = NULL
		};
		httpd_register_uri_handler(http_server_handle, &data_sensor_json);
