...
#include "../Library/wifi_reset_button.h"
#include "../Library/mlx90614_API.h"
#include "../Library/mlx90614_Driver.h"
#include "../Library/mq_sensors.h"
#include "../Library/datamanager.h"
#include "../Library/devicemanager.h"
                ...
                if (xQueueReceive(dataSensorSentToMQTT_queue, (void *)&dataSensorReceiveFromQueue, portMAX_DELAY) == pdPASS)
                {
                    ...
                    if (xSemaphoreTake(sentDataToMQTT_semaphore, portMAX_DELAY) == pdTRUE)
                    {
                        ...
                        sprintf(mqttMessage, formatDataSensorString, MAC_address[0],
                                ...
                                MAC_address[2],
                                MAC_address[3],
                                dataSensorReceiveFromQueue.temperature_object,
                                dataSensorReceiveFromQueue.temperature_ambient,
                                dataSensorReceiveFromQueue.LPG,
                                dataSensorReceiveFromQueue.CH4);
    ...
    for (;;)
    {
        ...
        if (xSemaphoreTake(getDataSensor_semaphore, portMAX_DELAY))
        {
            ...
            printf("%s,%llu,%.2f,%.2f,%.2f,%d,%d,%d\n", CONFIG_NAME_DEVICE,
                   ...
                   dataSensorTemp.temperature_object,
                   dataSensorTemp.temperature_ambient,
                   dataSensorTemp.LPG,
                   dataSensorTemp.CH4);


            ESP_ERROR_CHECK_WITHOUT_ABORT(moduleErrorTemp.ds3231Error);
            ESP_ERROR_CHECK_WITHOUT_ABORT(moduleErrorTemp.mlx90614Error);
            ESP_ERROR_CHECK_WITHOUT_ABORT(moduleErrorTemp.mq7Error);
            ESP_ERROR_CHECK_WITHOUT_ABORT(moduleErrorTemp.mq6Error);
            ...
            if (moduleError_queue != NULL &&
                (moduleErrorTemp.ds3231Error != ESP_OK ||
                 moduleErrorTemp.mlx90614Error != ESP_OK ||
                 moduleErrorTemp.mq7Error != ESP_OK ||
                 moduleErrorTemp.mq6Error !=ESP_OK))
            {
        ...
        if (uxQueueMessagesWaiting(dataSensorSentToSD_queue) != 0) // Check if dataSensorSentToSD_queue is empty
        {
            if (xQueueReceive(dataSensorSentToSD_queue, (void *)&dataSensorReceiveFromQueue, WAIT_10_TICK * 50) == pdPASS) // Get data sesor from queue
            {
                ...
                if (xSemaphoreTake(writeDataToSDcard_semaphore, portMAX_DELAY) == pdTRUE)
                {
                    ...
                    errorCode_t = sdcard_writeDataToFile(nameFileSaveData, "%s,%llu,%.2f,%.2f,%.2f,%.2f\n", CONFIG_NAME_DEVICE,
